...
(function () {
  ...
  var MIRROR_FRAG = [
    ...
    '',
    '  // ── Volumetric fog vignette (screen space) ──',
    '  vec2 screenUV = gl_FragCoord.xy / uResolution;',
    '  vec2 screenCenter = screenUV - 0.5;',
    '  screenCenter.x *= uResolution.x / uResolution.y;',
    '  float screenDist = length(screenCenter);',
    '  float fogEdge = smoothstep(0.25, 0.55, screenDist);',
    '  float fogAngle = atan(screenCenter.y, screenCenter.x);',
    '',
    '  // Clockwise-drifting fog layers',
    '  float fA = fogAngle - uTime * 0.12;',
    '  float fog1 = sin(fA * 4.0 + screenDist * 10.0 - uTime * 0.25) * 0.5 + 0.5;',
    '  float fog2 = sin(fA * 6.0 - screenDist * 8.0 + uTime * 0.18) * 0.5 + 0.5;',
    '  float fog3 = sin(fA * 2.0 + screenDist * 14.0 - uTime * 0.35) * 0.5 + 0.5;',
