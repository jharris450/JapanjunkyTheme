...
(function () {
  ...
  var MIRROR_FRAG = [
    ...
    '  float fogEdge = smoothstep(0.2, 0.6, dist);',
    '  float fogAngle = atan(center.y, center.x);',
    '',
    '  // Clockwise-drifting fog layers',
    '  float fA = fogAngle - uTime * 0.12;',
    '  float fog1 = sin(fA * 4.0 + dist * 10.0 - uTime * 0.25) * 0.5 + 0.5;',
    '  float fog2 = sin(fA * 6.0 - dist * 8.0 + uTime * 0.18) * 0.5 + 0.5;',
    '  float fog3 = sin(fA * 2.0 + dist * 14.0 - uTime * 0.35) * 0.5 + 0.5;',
    ...
    '  color = mix(color, fogColor, fogDensity * 0.7);',
    '  color += fogColor * fogDensity * 0.15;',
    '',
    '  // ── Tsuno silhouette ──',
    '  if (uGhostLoaded > 0.5) {',
