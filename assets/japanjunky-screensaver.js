...
(function () {
  ...
  var MEMPHIS_FRAG = [
    ...
    '// Each stroke pulses its own thickness/length on a per-cell phase so',
    '// the field shimmers like the reference gif rather than throbbing in',
    '// unison.',
    'float stroke(vec2 p) {',
    '  vec2 cell = floor(p);',
    ...
    '  float phase = mhash2(cell * 1.93 + 4.21) * 6.2832;',
    '  float pulse = 0.5 + 0.5 * sin(uTime * 4.8 + phase);',
    '  float baseLen = 0.14 + 0.10 * mhash2(cell * 2.71);',
    '  float halfLen = baseLen * mix(0.45, 1.15, pulse);',
    '  float x = clamp(r.x, -halfLen, halfLen);',
    '  float y = 0.55 * x * x - 0.025;',
    '  float d = length(r - vec2(x, y));',
    '  float thickness = mix(0.045, 0.105, pulse);',
