...
(function () {
  ...
  var TUNNEL_FRAG = [
    ...
    '  float phase = fract(uTime * 0.05);',
    '  float mscale = mix(2.5, 18.0, phase);',
    '  vec2 mp = vec2(angle * 6.2832 * 1.4, depth * 10.0) * (mscale / 8.0);',
    '  vec2 mw = vec2(',
    '    mfbm(mp * 0.7 + vec2(uTime * 0.15, 0.0)),',
    '    mfbm(mp * 0.7 + vec2(5.2, 1.3) - vec2(0.0, uTime * 0.12))',
    '  );',
    '  mp += mw * 2.2;',
    '  float mn = mfbm(mp * 1.1);',
    '  float mmask = smoothstep(0.46, 0.54, mn);',
    '  // Bright cream ink marks overlay tunnel — high contrast so they',
    '  // survive VGA quantization and read clearly against the swirl.',
    '  vec3 memphisInk = vec3(0.95, 0.85, 0.55);',
    '  color = mix(color, memphisInk, mmask * 0.85);',
    '',
