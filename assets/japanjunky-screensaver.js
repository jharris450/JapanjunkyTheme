...
(function () {
  ...
  var TUNNEL_FRAG = [
    ...
    '',
    '  // Memphis contour marks baked onto tunnel walls. Continuous',
    '  // big→small via crossfade of two phase-offset copies (no reverse).',
    '  float mt = uTime * 0.08;',
    '  float mp1 = fract(mt);',
    '  float mp2 = fract(mt + 0.5);',
    '  float ms1 = mix(3.0, 14.0, mp1);',
    '  float ms2 = mix(3.0, 14.0, mp2);',
    '  float mw1f = sin(mp1 * 3.14159);',
    '  float mw2f = sin(mp2 * 3.14159);',
    '  vec2 mbase = vec2(angle * 6.2832 * 1.3, depth * 10.0);',
    '  vec2 mwarp = vec2(',
    '    mfbm(mbase * 0.08 + vec2(uTime * 0.12, 0.0)),',
    '    mfbm(mbase * 0.08 + vec2(5.2, 1.3) - vec2(0.0, uTime * 0.09))',
    '  );',
    '  vec2 mpA = mbase * (ms1 / 8.0) + mwarp * 2.0;',
    '  vec2 mpB = mbase * (ms2 / 8.0) + mwarp * 2.0;',
    '  float mnA = mfbm(mpA);',
    '  float mnB = mfbm(mpB);',
    '  float thk = 0.05;',
    '  float maA = 1.0 - smoothstep(thk * 0.4, thk, abs(mnA - 0.5));',
    '  float maB = 1.0 - smoothstep(thk * 0.4, thk, abs(mnB - 0.5));',
    '  float mmask = (maA * mw1f + maB * mw2f) / max(mw1f + mw2f, 0.001);',
    '  // Fade ink out near the far opening so no marks fall inside',
    '  // the portal center.',
    '  mmask *= smoothstep(1.0, 0.55, depth);',
    '  color = mix(color, vec3(0.02, 0.01, 0.0), mmask * 0.9);',
    '',
  ...
  var MEMPHIS_FRAG = [
    ...
    '}',
    '',
    '// Leopard-print-ish curved dash. Per-cell random rotation, offset,',
    '// and length give a dense, organic look rather than a noodle field.',
    'float stroke(vec2 p) {',
    '  vec2 cell = floor(p);',
    '  vec2 local = p - cell - 0.5;',
    '  float seed = mhash2(cell * 7.13 + 0.17);',
    '  float angle = seed * 6.2832;',
    '  float ca = cos(angle);',
    '  float sa = sin(angle);',
    '  vec2 off = vec2(mhash2(cell * 3.11), mhash2(cell * 5.97)) * 0.55 - 0.275;',
    '  vec2 q = local - off;',
    '  vec2 r = vec2(q.x * ca + q.y * sa, -q.x * sa + q.y * ca);',
    '  float halfLen = 0.14 + 0.10 * mhash2(cell * 2.71);',
    '  float x = clamp(r.x, -halfLen, halfLen);',
    '  float y = 0.55 * x * x - 0.025;',
    '  float d = length(r - vec2(x, y));',
    ...
    '  float rad = length(uv);',
    '',
    '  // Continuous shrink via crossfade of two phase-offset copies.',
    '  // Each copy grows scale (big→small marks) then fades out as the',
    '  // other copy ramps in. No hitch, no reverse.',
    '  float t = uTime * 0.09;',
    '  float p1 = fract(t);',
    '  float p2 = fract(t + 0.5);',
    '  float s1 = mix(4.0, 22.0, p1);',
    '  float s2 = mix(4.0, 22.0, p2);',
    '  float w1 = sin(p1 * 3.14159);',
    '  float w2 = sin(p2 * 3.14159);',
    '  float m1 = strokeField(uv * s1);',
    '  float m2 = strokeField(uv * s2);',
    '  float mask = (m1 * w1 + m2 * w2) / max(w1 + w2, 0.001);',
    '',
