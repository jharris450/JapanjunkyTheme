...
(function () {
  ...
  var TUNNEL_FRAG = [
    ...
    '',
    '  // ── Volumetric fog: soft depth haze ──────────────────────',
    '  // Depth-based only — NO angular sine pattern (that read as',
    '  // squiggles taking over the scene). Tinted LIGHTER than the',
    '  // walls so the haze reads instead of blending in. A gentle',
    '  // global brightness breath keeps it feeling alive.',
    '  float fogFar = 1.0 - exp(-depth * 2.0);',
    '  float fogMid = exp(-pow((depth - 0.5) * 2.4, 2.0)) * 0.5;',
    '  float fogDensity = clamp(fogFar * 0.7 + fogMid, 0.0, 1.0);',
    '  fogDensity *= 0.85 + 0.15 * sin(uTime * 0.4);',
    '  vec3 fogColor = mix(vec3(1.0, 0.85, 0.6), vec3(0.85, 0.7, 1.0), depthMix);',
    '  color = mix(color, fogColor, fogDensity * 0.4);',
    '',
