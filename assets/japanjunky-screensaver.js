...
(function () {
  ...
  var MEMPHIS_FRAG = [
    ...
    '  float rad = length(uv);',
    '',
    '  // Pulse: scale throbs around a base value, intensity throbs in sync.',
    '  // Mimics drum-beat pulse from reference gif — marks breathe in/out',
    '  // rhythmically rather than continuously shrinking.',
    '  float bpm = 1.6;',
    '  float pulse = 0.5 + 0.5 * sin(uTime * bpm * 6.2832);',
    '  float scale = mix(11.0, 15.0, pulse);',
    '  float mask = strokeField(uv * scale);',
    '  mask *= mix(0.78, 1.18, pulse);',
