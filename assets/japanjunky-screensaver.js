...
(function () {
  ...
  var TUNNEL_FRAG = [
    ...
    '',
    '  // Memphis squiggle contour marks — baked onto tunnel walls so',
    '  // they show through the wrapping cylinder. Animates big→small.',
    '  // Iso-lines of warped fbm give thin curvy strokes (not blobs).',
    '  float mosc = 0.5 - 0.5 * cos(uTime * 0.25);',
    '  float mscale = mix(3.0, 14.0, mosc);',
    '  vec2 mp = vec2(angle * 6.2832 * 1.3, depth * 10.0) * (mscale / 8.0);',
    '  vec2 mw = vec2(',
    '    mfbm(mp * 0.6 + vec2(uTime * 0.12, 0.0)),',
    '    mfbm(mp * 0.6 + vec2(5.2, 1.3) - vec2(0.0, uTime * 0.09))',
    '  );',
    '  mp += mw * 2.0;',
    '  float mn = mfbm(mp);',
    '  float ringDist = abs(mn - 0.5);',
    '  float thickness = 0.05;',
    '  float mmask = 1.0 - smoothstep(thickness * 0.4, thickness, ringDist);',
    '  // Near-black ink strokes — strong contrast against swirl without',
  ...
  var MEMPHIS_FRAG = [
    ...
    '}',
    '',
    '// Stroke pattern — each unit cell draws one short curved dash',
    '// at a random angle and offset. Two overlapping tilings fill gaps.',
    'float stroke(vec2 p) {',
    '  vec2 cell = floor(p);',
    ...
    '  float ca = cos(angle);',
    '  float sa = sin(angle);',
    '  vec2 off = vec2(mhash2(cell * 3.11), mhash2(cell * 5.97)) * 0.4 - 0.2;',
    '  vec2 q = local - off;',
    '  vec2 r = vec2(q.x * ca + q.y * sa, -q.x * sa + q.y * ca);',
    '  float halfLen = 0.32;',
    '  float x = clamp(r.x, -halfLen, halfLen);',
    '  float y = 0.35 * x * x - 0.04;',
    '  float d = length(r - vec2(x, y));',
    '  float thickness = 0.07;',
    '  return 1.0 - smoothstep(thickness * 0.6, thickness, d);',
    '}',
    '',
    ...
    '  uv.x *= uAspect;',
    '',
    '  // Seamless big→small→big via sine oscillation (no hitch).',
    '  float osc = 0.5 - 0.5 * cos(uTime * 0.2);',
    '  float scale = mix(6.0, 26.0, osc);',
    '',
    '  vec2 p = uv * scale;',
    '  float mask = strokeField(p);',
    '',
    '  // Amber bg + black ink — Memphis on warm portal tone.',
