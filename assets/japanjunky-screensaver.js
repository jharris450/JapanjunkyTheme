...
(function () {
  ...
  var TUNNEL_FRAG = [
    ...
    '',
    '  // ── Volumetric fog: drifting wisps + depth haze ──────────',
    '  // Slow low-frequency curls (billows, not strands) drifting down',
    '  // the tunnel, plus a soft depth haze. Tinted LIGHTER than the',
    '  // walls so it actually reads as fog instead of blending in.',
    '  float fA = a - uTime * 0.15;',
    '  float fD = depth * 4.0 - uTime * 0.22;',
    '  float w1 = sin(fA * 2.0 + fD * 1.6) * 0.5 + 0.5;',
    '  float w2 = sin(fA * 3.0 - fD * 1.1 + 1.7) * 0.5 + 0.5;',
    '  float w3 = sin(fA * 1.0 + fD * 2.4 - 0.6) * 0.5 + 0.5;',
    '  float wisp = w1 * 0.5 + w2 * 0.3 + w3 * 0.2;',
    '  wisp = smoothstep(0.25, 0.95, wisp);',
    '  // Fade fog in past the near rim and out before the far glow',
    '  float fogWindow = smoothstep(0.04, 0.40, depth) * smoothstep(1.0, 0.5, depth);',
    '  vec3 fogTint = mix(vec3(1.0, 0.82, 0.55), vec3(0.82, 0.66, 0.98), depthMix);',
    '  float haze = (1.0 - exp(-depth * 1.8)) * 0.30;',
    '  color = mix(color, fogTint, clamp(haze, 0.0, 1.0) * fogWindow);',
    '  color += fogTint * wisp * fogWindow * 0.45;',
    '',
